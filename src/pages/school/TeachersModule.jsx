...
const TeachersModule = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (editingTeacher) {
                // UPDATE FLOW
                await updateTeacherDetails(schoolId, editingTeacher.uid || editingTeacher.id, {
                    name: data.name,
                    subjects: subjects
                }, selectedClassIds);
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
                    <div className="bg-white rounded-[40px] p-10 max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-200 border border-blue-100 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <h2 className="text-3xl font-black mb-8 text-gray-900 tracking-tight italic">
                                {editingTeacher ? 'Edit Faculty' : 'Invite Faculty'}
                            </h2>
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                <div className={editingTeacher ? "opacity-50 pointer-events-none" : ""}>
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Professional Email</label>
                                    <input {...register("email", { required: !editingTeacher })} type="email" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:bg-white focus:border-blue-300 font-bold text-sm" placeholder="faculty@school.edu" />
                                    {editingTeacher && <p className="text-[9px] text-gray-400 mt-1 ml-1">Email cannot be changed after registration.</p>}
                                </div>
                                <div>
                                ...
                                <div className="flex justify-end gap-3 pt-6">
                                    <button
                                        ...
                                        className="px-6 py-3 text-gray-400 font-black uppercase text-[10px] tracking-widest hover:bg-gray-50 rounded-xl transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button type="submit" className="px-10 py-4 bg-primary-600 text-white rounded-[24px] hover:bg-primary-700 font-black text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-primary-500/20 active:scale-95 transition-all flex items-center gap-2">
                                        {editingTeacher ? <Edit3 className="w-4 h-4" /> : <Mail className="w-4 h-4" />}
                                        {editingTeacher ? 'Update Profile' : 'Dispatch Link'}
                                    </button>
                                </div>
                            </form>
                        </div>
